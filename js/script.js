const filterByType = (type, ...values) => values.filter(value => typeof value === type),
	//(type, ...values) = в функцию filterByType принимаем тип и другие аргумены, которые собираются в массив values, функция возвращает новый результирующий массив
	// values.filter() = проходимся по массиву values методом filter  
	...
	hideAllResponseBlocks = () => { //функция для скрытия элементов на странице
		const responseBlocksArray = Array.from(document.querySelectorAll('div.dialog__response-block')); //получаем со страницы все элементы div с классом .dialog__response-block, формируем из них массив
		responseBlocksArray.forEach(block => block.style.display = 'none'); //каждому элементу массива добавляем стиль display = 'none', чтобы не показывать блок на странице
	},

	showResponseBlock = (blockSelector, msgText, spanSelector) => { //функция для отображения элементов на странице, 
		...
		hideAllResponseBlocks(); //сначала скрываем все элементы
		...
		if (spanSelector) { //если был передан spanSelector и он не является пустым или null
			document.querySelector(spanSelector).textContent = msgText; //то в элемент с этим селектором мы помещаем текст, полученный вторым параметром в функцию 
		}
	},
	...
	tryFilterByType = (type, values) => {
		...
		try { //попробуем сделать следующее: 
			...
			showError(`Ошибка: ${e}`); //то мы показываем её на странице, вызывая функцию showError и передавая в неё объект ошибки
		}
	};
...
filterButton.addEventListener('click', e => { //добавляем обработчик по клику на кнопку "фильтровать"
	...
	if (dataInput.value === '') { //если ничего не ввели в поле "Данные"
		dataInput.setCustomValidity('Поле не должно быть пустым!'); //подсвечиваем поле и говорим, что нужно заполнить поле
		...
		e.preventDefault(); //отменяем стандартное поведение элемента (в данном случае кнопка submit отправляет форму и перезагружает страницу)
